...
| `components/ResultEditor.tsx` | Editable draft + copy + pipeline insight |
| `app/api/generate/route.ts` | Multipart entry point; parses file, runs pipeline |
| `lib/resume-parser.ts` | PDF/DOCX/TXT → plain text |
| `lib/llm.ts` | Anthropic client + JSON-completion helpers |
| `lib/prompts.ts` | The four prompt-chain step builders |
| `lib/pipeline.ts` | Orchestrates the chain |
